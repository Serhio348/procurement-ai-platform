...
- [`docs/architecture/STAGE-110.md`](docs/architecture/STAGE-110.md) — адаптивная вёрстка ≤768px: гибридная навигация (top bar ☰ + bottom tabs + drawer со всеми разделами), однопанельный list↔detail (карточка над списком по `:has()`/`is-triage-*`), тач-цели ≥44px, safe-area + мета для будущего PWA (R32)
