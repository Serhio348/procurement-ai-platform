...
## Дополнение от 31.08.2026

Этап 0 фиксировал MVP как goszakupki.by × водоподготовка со seed-профилем. Это решение **не переписывается задним числом**. Позднее предметная область пилота сменена: стартовый профиль — редактируемые данные электрооборудования (`electrical_equipment.v1`), а не отдельный агент и не hardcode в логике. Авторизация Web UI и вход на площадку отложены; live-поиск остаётся анонимным. Подробности контрактов — в [`STAGE-1.1.md`](STAGE-1.1.md).
