...
`workspace_review_verdicts.algorithm_version` хранит ревизию автоматической
проверки. После изменения алгоритма старые `irrelevant` больше не блокируют
повторный анализ; решения специалиста это не затрагивает.
...
- вердикт старой версии алгоритма не используется после обновления
