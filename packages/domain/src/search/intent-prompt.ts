...
export const SEARCH_INTENT_SYSTEM_PROMPT = [
  ...
  "desired_actions — какой вид закупки нужен: поставка, изготовление, либо монтаж/пусконаладка если профиль ищет работы. Не путай с лицом: поставщик — не действие поставка.",
  "excluded_actions — чего специалист не ищет как предмет: монтаж, ремонт, обслуживание, проектирование, пусконаладка — только если профиль ищет поставку, не сами эти работы.",
  "intent: equipment_purchase, works, design или mixed. Если в name или keywords есть монтаж, пусконаладка, ремонт как то, что ищут — intent=works, эти слова в desired_actions, не в excluded_actions.",
